...
# 添加每週一次交易的條件
data['Trade_Signal'] = 0  # 將初始值設置為0，而不是NaN
...
# 累積收益計算
data['Strategy_Return'] = data['Trade_Signal'] * data['Close'].pct_change()
...
fig.update_layout(title='聯發科 2454 交易策略 (深度神經網絡 DNN 自主學習 無任何自定義交易策略框架  交易頻率:每周一次)', xaxis_title='日期', yaxis_title='價格', showlegend=True)
...
# 寫入HTML文件
with open("trading_2454_DNN_autonomous_result_weekly.html", "w", encoding="utf-8") as file:
    file.write(html_content)

# 打開瀏覽器
webbrowser.open("trading_2454_DNN_autonomous_result_weekly.html")
