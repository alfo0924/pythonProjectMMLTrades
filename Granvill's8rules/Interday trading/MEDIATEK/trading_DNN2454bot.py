...
data['Buy_Signal'] = np.where(
    ...
    (
            ((data['Close'] > data['SMA_200'].shift(1)) & (data['SMA_200'] <= data['SMA_200'].shift(1))) |
            ((data['Close'] > data['SMA_200'].shift(1)) & (data['Close'] > data['SMA_200']))
    ),
    1, 0
)
...
fig.update_layout(title='聯發科 2454 交易策略 (深度神經網絡 DNN + 格蘭碧8大法則 均線:200均 交易頻率:一天多次)', xaxis_title='日期', yaxis_title='價格', showlegend=True)
...
# 寫入HTML文件
with open("trading_Granvills8rules_2454_DNN_result.html", "w", encoding="utf-8") as file:
    file.write(html_content)

# 打開瀏覽器
webbrowser.open("trading_Granvills8rules_2454_DNN_result.html")
